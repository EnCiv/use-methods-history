...
export function useMethods(methodsObj, initialState, umsKey) {
    ...
    const [_this]=useState({methodChild: undefined, umsKey})
    if(!_this.umsKey) {
    ...
    if(umsKey && !_this.methodChild){ // if no umsKey no need for this to find previous state by that key
    ...
    if(!_this.methodChild){
        _this.methodChild={dispatch, methodState, umsKey: _this.umsKey, unmounted: false}
        useMethods.children.add(_this.methodChild)
    }
    ...
    useEffect(()=>{
        ...
        return ()=>{
            _this.methodChild.unmounted=true

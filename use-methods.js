...
export function useMethods(methodsObj, initialState, umsKey, deps=[]) {
    ...
    const [_this]=useState(()=>{
        ...
        return {
            umsKey: umsKey || (!useMethods.children.size? 'ums-top' : 'ums__'+useMethods.nextUniqueUmsKey++),
    ...
    useEffect(()=>{
        ...
        return ()=>{
            ...
            //!! if unmounting with an autogenerated umsKey then delete it
            if(_this.umsKey.startsWith('ums__')) useMethods.children.delete(_this)
        }
    },[])

...
function reducer(state, action) {
    ...
    if (!useMethodsHistory.inPopState) {
...
export function useMethodsHistory(methodsObj, initialState, umhKey, deps = []) {
    ...
    const [_this] = useState(() => {
        ...
        return {
            umhKey:
                umhKey ||
                (!useMethodsHistory.children.size ? "umh-top" : "umh__" + useMethodsHistory.nextUniqueumhKey++),
    ...
    if (!_this.dispatch) {
        ...
        useMethodsHistory.children.add(_this)
    }
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            if (_this.umhKey.startsWith("umh__")) useMethodsHistory.children.delete(_this)
        }
    }, [])
